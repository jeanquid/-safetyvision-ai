import React, { useState, useRef } from 'react';
import { useAuth } from '../contexts/AuthContext';
import { Camera, Upload, Loader2, AlertTriangle, CheckCircle, Send, FileText } from 'lucide-react';
import { useEffect } from 'react';

const RISK_META: Record<string, { color: string; bg: string; label: string }> = {
    alto: { color: 'text-red-400', bg: 'bg-red-500/10 border-red-500/20', label: 'ALTO' },
    medio: { color: 'text-amber-400', bg: 'bg-amber-500/10 border-amber-500/20', label: 'MEDIO' },
    bajo: { color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20', label: 'BAJO' },
};

const CAT_ICONS: Record<string, string> = { epp: '🦺', condiciones: '🏭', comportamiento: '🚧' };

interface Props {
    onComplete: () => void;
}

export const NewInspection: React.FC<Props> = ({ onComplete }) => {
    const { authFetch, user } = useAuth();
    const fileRef = useRef<HTMLInputElement>(null);

    const [step, setStep] = useState<'form' | 'analyzing' | 'results'>('form');
    const [plant, setPlant] = useState('Planta Norte');
    const [sector, setSector] = useState('');
    const [operator, setOperator] = useState(user?.displayName || '');
    const [description, setDescription] = useState('');
    const [imagePreview, setImagePreview] = useState<string | null>(null);
    const [imageBase64, setImageBase64] = useState<string | null>(null);
    const [mimeType, setMimeType] = useState('image/jpeg');
    const [risks, setRisks] = useState<any[]>([]);
    const [aiModel, setAiModel] = useState('');
    const [saving, setSaving] = useState(false);
    const [saved, setSaved] = useState(false);
    const [error, setError] = useState('');
    const [plants, setPlants] = useState<{ name: string; sectors: string[] }[]>([]);
    const [configLoading, setConfigLoading] = useState(true);

    // Cargar configuración al montar:
    useEffect(() => {
        const loadConfig = async () => {
            try {
                const res = await authFetch('/api/config');
                const data = await res.json();
                if (data.ok && data.tenant?.plants) {
                    setPlants(data.tenant.plants);
                    // Setear la primera planta como default
                    if (data.tenant.plants.length > 0 && !plant) {
                        setPlant(data.tenant.plants[0].name);
                    }
                }
            } catch (err) {
                console.error('Failed to load config:', err);
            }
            setConfigLoading(false);
        };
        loadConfig();
    }, []);

    const handleFile = (e: React.ChangeEvent<HTMLInputElement>) => {
        const file = e.target.files?.[0];
        if (!file) return;
        setMimeType(file.type || 'image/jpeg');
        const reader = new FileReader();
        reader.onload = (ev) => {
            const full = ev.target?.result as string;
            setImagePreview(full);
            setImageBase64(full.split(',')[1]); // strip data:...;base64,
        };
        reader.readAsDataURL(file);
    };

    const handleAnalyze = async () => {
        if (!imageBase64 && !description) {
            setError('Subí una foto o describí la situación');
            return;
        }
        setError('');
        setStep('analyzing');

        try {
            const body: any = { plant, sector };
            if (imageBase64) {
                body.imageBase64 = imageBase64;
                body.mimeType = mimeType;
            } else {
                body.description = description;
            }

            const res = await authFetch('/api/inspections/analyze', {
                method: 'POST',
                body: JSON.stringify(body),
            });
            const data = await res.json();

            if (!res.ok || !data.ok) throw new Error(data.error || 'Analysis failed');

            setRisks(data.risks || []);
            setAiModel(data.model || 'unknown');
            setStep('results');
        } catch (err: any) {
            setError(err.message);
            setStep('form');
        }
    };

    const handleSave = async () => {
        setSaving(true);
        try {
            const highestLevel = risks.reduce((h: string, r: any) => {
                const order: Record<string, number> = { alto: 3, medio: 2, bajo: 1 };
                return (order[r.level] || 0) > (order[h] || 0) ? r.level : h;
            }, 'bajo');

            // Generar acción legible
            const riskSummary = risks.length === 1
                ? `Corregir: ${risks[0].description}`
                : `Corregir ${risks.length} riesgos detectados: ${risks
                    .slice(0, 3)
                    .map((r: any) => r.recommendation || r.description)
                    .join('; ')}${risks.length > 3 ? ` (+${risks.length - 3} más)` : ''}`;

            const task = {
                action: riskSummary,
                responsible: 'Supervisor de turno',
                deadline: highestLevel === 'alto' ? '4 hs' : highestLevel === 'medio' ? '24 hs' : '48 hs',
                status: 'pendiente',
            };

            const res = await authFetch('/api/inspections/create', {
                method: 'POST',
                body: JSON.stringify({
                    plant, sector, operator, risks, task,
                    imageBase64, mimeType, // Add for Phase 2 persistence
                    aiAnalysis: { model: aiModel, analyzedAt: new Date().toISOString() },
                }),
            });
            const data = await res.json();
            if (!res.ok || !data.ok) throw new Error(data.error || 'Save failed');

            setSaved(true);
            setTimeout(() => onComplete(), 1500);
        } catch (err: any) {
            setError(err.message);
        }
        setSaving(false);
    };

    // ── SUCCESS ──
    if (saved) {
        return (
            <div className="max-w-md mx-auto text-center py-20">
                <div className="text-5xl mb-4">✅</div>
                <h2 className="text-xl font-bold text-white mb-2">Inspección Guardada</h2>
                <p className="text-slate-500 text-sm">
                    La tarea correctiva fue asignada y las alertas enviadas.
                </p>
            </div>
        );
    }

    // ── FORM ──
    if (step === 'form') {
        return (
            <div className="max-w-xl mx-auto space-y-6">
                <div className="text-center">
                    <div className="text-5xl mb-2">📸</div>
                    <h2 className="text-xl font-bold text-white">Nueva Inspección</h2>
                    <p className="text-slate-500 text-sm mt-1">Subí una foto o describí la situación para que la IA analice</p>
                </div>

                {error && (
                    <div className="flex items-center gap-2 text-red-400 bg-red-500/10 border border-red-500/20 rounded-xl p-3 text-sm">
                        <AlertTriangle className="w-4 h-4 shrink-0" /> {error}
                    </div>
                )}

                <div className="grid grid-cols-2 gap-3">
                    <div>
                        <label className="block text-[10px] text-slate-500 uppercase tracking-wider mb-1">Planta</label>
                        <select value={plant} onChange={e => { setPlant(e.target.value); setSector(''); }}
                            className="w-full px-3 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500">
                            {plants.map(p => <option key={p.name}>{p.name}</option>)}
                        </select>
                    </div>
                    <div>
                        <label className="block text-[10px] text-slate-500 uppercase tracking-wider mb-1">Sector</label>
                        <select value={sector} onChange={e => setSector(e.target.value)}
                            className="w-full px-3 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500">
                            <option value="">Seleccionar sector...</option>
                            {(plants.find(p => p.name === plant)?.sectors || []).map(s =>
                                <option key={s}>{s}</option>
                            )}
                        </select>
                    </div>
                </div>

                <div>
                    <label className="block text-[10px] text-slate-500 uppercase tracking-wider mb-1">Operador / Reporta</label>
                    <input value={operator} onChange={e => setOperator(e.target.value)} placeholder="Nombre"
                        className="w-full px-3 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500" />
                </div>

                {/* Photo upload */}
                <div>
                    <input ref={fileRef} type="file" accept="image/*" capture="environment" onChange={handleFile} className="hidden" />
                    {imagePreview ? (
                        <div className="relative rounded-xl overflow-hidden border border-slate-700">
                            <img src={imagePreview} alt="preview" className="w-full max-h-64 object-cover" />
                            <button onClick={() => { setImagePreview(null); setImageBase64(null); }}
                                className="absolute top-2 right-2 bg-black/60 text-white rounded-full w-7 h-7 flex items-center justify-center text-xs hover:bg-black/80">✕</button>
                        </div>
                    ) : (
                        <div onClick={() => fileRef.current?.click()}
                            className="border-2 border-dashed border-blue-500/30 rounded-xl p-10 text-center cursor-pointer hover:border-blue-500/60 hover:bg-blue-500/5 transition-colors">
                            <Camera className="w-8 h-8 text-blue-400 mx-auto mb-2" />
                            <div className="text-blue-400 font-semibold text-sm">Subir foto de inspección</div>
                            <div className="text-slate-500 text-xs mt-1">Tocá para abrir la cámara o elegir archivo</div>
                        </div>
                    )}
                </div>

                <div className="text-center text-slate-600 text-xs">— o describí la situación —</div>

                <textarea value={description} onChange={e => setDescription(e.target.value)}
                    placeholder="Ej: Operario sin casco trabajando cerca de maquinaria pesada, cables sueltos en el piso..."
                    rows={3}
                    className="w-full px-3 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500 resize-none" />

                <button onClick={handleAnalyze}
                    className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl transition-colors flex items-center justify-center gap-2">
                    <Send className="w-4 h-4" /> Analizar con IA
                </button>
            </div>
        );
    }

    // ── ANALYZING ──
    if (step === 'analyzing') {
        return (
            <div className="max-w-md mx-auto text-center py-20">
                <div className="relative w-20 h-20 mx-auto mb-6">
                    <div className="w-20 h-20 rounded-full border-[3px] border-transparent border-t-blue-500 animate-spin" />
                    <div className="absolute inset-0 flex items-center justify-center text-3xl">🤖</div>
                </div>
                <h2 className="text-lg font-bold text-white mb-2">Analizando...</h2>
                <p className="text-slate-500 text-sm">Gemini AI está detectando riesgos de seguridad</p>
                <div className="mt-8 space-y-2 text-left max-w-xs mx-auto">
                    {['Procesando imagen...', 'Detectando EPP...', 'Evaluando condiciones...', 'Clasificando riesgos...'].map((t, i) => (
                        <div key={i} className="flex items-center gap-3 bg-slate-900/50 rounded-lg p-3 animate-pulse" style={{ animationDelay: `${i * 0.3}s` }}>
                            <div className="w-1.5 h-1.5 rounded-full bg-blue-500" />
                            <span className="text-xs text-slate-500">{t}</span>
                        </div>
                    ))}
                </div>
            </div>
        );
    }

    // ── RESULTS ──
    const highestLevel = risks.reduce((h: string, r: any) => {
        const order: Record<string, number> = { alto: 3, medio: 2, bajo: 1 };
        return (order[r.level] || 0) > (order[h] || 0) ? r.level : h;
    }, 'bajo');

    return (
        <div className="max-w-2xl mx-auto space-y-6">
            <div className="text-center">
                <div className="text-4xl mb-2">✅</div>
                <h2 className="text-xl font-bold text-white">Análisis Completado</h2>
                <p className="text-slate-500 text-sm mt-1">
                    Se detectaron <span className={RISK_META[highestLevel]?.color || 'text-white'}>{risks.length} riesgo{risks.length !== 1 ? 's' : ''}</span> · Modelo: <span className="text-slate-400 font-mono text-xs">{aiModel}</span>
                </p>
            </div>

            {error && (
                <div className="text-red-400 bg-red-500/10 border border-red-500/20 rounded-xl p-3 text-sm">{error}</div>
            )}

            {/* Detections */}
            <div className="space-y-2">
                {risks.map((r: any, i: number) => {
                    const meta = RISK_META[r.level] || RISK_META.medio;
                    return (
                        <div key={i} className={`flex items-center gap-4 p-4 bg-slate-900/50 border rounded-xl ${meta.bg}`}
                            style={{ borderLeftWidth: 3, borderLeftColor: meta.color.includes('red') ? '#EF4444' : meta.color.includes('amber') ? '#F59E0B' : '#22C55E' }}>
                            <span className="text-2xl">{CAT_ICONS[r.category] || '⚠️'}</span>
                            <div className="flex-1">
                                <div className="text-sm text-white font-semibold">{r.description}</div>
                                <div className="text-xs text-slate-500 mt-0.5">{r.category === 'epp' ? 'Seguridad Personal' : r.category === 'condiciones' ? 'Condiciones Inseguras' : 'Comportamiento Riesgoso'}</div>
                                {r.recommendation && (
                                    <div className="text-xs text-blue-400/70 mt-1">→ {r.recommendation}</div>
                                )}
                            </div>
                            <div className="text-right">
                                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${meta.bg} ${meta.color}`}>{meta.label}</span>
                                <div className="text-[10px] text-slate-500 mt-1 font-mono">{r.confidence}% conf.</div>
                            </div>
                        </div>
                    );
                })}
            </div>

            {risks.length === 0 && (
                <div className="text-center py-8 bg-emerald-500/5 border border-emerald-500/20 rounded-xl">
                    <CheckCircle className="w-8 h-8 text-emerald-400 mx-auto mb-2" />
                    <p className="text-emerald-400 font-semibold">No se detectaron riesgos</p>
                    <p className="text-slate-500 text-xs mt-1">La zona inspeccionada parece segura</p>
                </div>
            )}

            {/* Auto task */}
            {risks.length > 0 && (
                <div className="bg-blue-500/5 border border-blue-500/15 rounded-xl p-5">
                    <div className="flex items-center gap-2 text-blue-400 font-bold text-sm mb-3">
                        <FileText className="w-4 h-4" /> Tarea Correctiva Generada
                    </div>
                    <div className="grid grid-cols-2 gap-4 text-xs">
                        <div>
                            <div className="text-slate-500 mb-0.5">Acción</div>
                            <div className="text-slate-300">{risks.map((r: any) => r.description).join('; ')}</div>
                        </div>
                        <div>
                            <div className="text-slate-500 mb-0.5">Responsable</div>
                            <div className="text-slate-300">Supervisor de turno</div>
                        </div>
                        <div>
                            <div className="text-slate-500 mb-0.5">Plazo</div>
                            <div className={RISK_META[highestLevel]?.color + ' font-bold'}>
                                {highestLevel === 'alto' ? '4 hs' : highestLevel === 'medio' ? '24 hs' : '48 hs'}
                            </div>
                        </div>
                        <div>
                            <div className="text-slate-500 mb-0.5">Alerta</div>
                            <div className="text-slate-300">📲 WhatsApp + Email</div>
                        </div>
                    </div>
                </div>
            )}

            <button onClick={handleSave} disabled={saving || risks.length === 0}
                className="w-full py-3 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-700 disabled:text-slate-500 text-white font-bold rounded-xl transition-colors flex items-center justify-center gap-2">
                {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />}
                {saving ? 'Guardando...' : 'Guardar Inspección'}
            </button>
        </div>
    );
};
